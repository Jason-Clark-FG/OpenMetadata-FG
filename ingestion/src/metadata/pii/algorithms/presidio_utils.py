...
@recognizer_factories.add(CreditCardRecognizer)
def credit_card_factory(**kwargs) -> CreditCardRecognizer:
...
@recognizer_factories.add(UsLicenseRecognizer)
def us_license_factory(**kwargs) -> UsLicenseRecognizer:
    return UsLicenseRecognizer(patterns=patterns.us_driving_license, **kwargs)


@recognizer_factories.add(AuTfnRecognizer)
    ...
    for cls in _get_all_entity_recognizer_classes():
        if issubclass(cls, PatternRecognizer):
            try:
                ...
                factory = recognizer_factories.get(cls, cls)
